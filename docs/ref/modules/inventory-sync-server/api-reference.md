...
| `DELETE` | `/agents` | `200` | Deletes every document of the agent named by `X-Wazuh-Agent-Id` across `wazuh-states-*`, `wazuh-agent-config` and `wazuh-agent-stats` (this cluster's scope). Each index is refreshed before the delete-by-query, so documents the agent's last session wrote inside the refresh interval are not missed. Deferred to the agent's worker shard, so it orders after that agent's in-flight sessions; `200` means every delete-by-query was flushed. UDS-local only: the production caller is authd. |
