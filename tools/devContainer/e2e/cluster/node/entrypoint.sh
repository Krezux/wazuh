...
# Indexer certificates from the mounted bundle.
install -d -o wazuh-manager -g wazuh-manager -m 750 "$CERT_DST"
install -o wazuh-manager -g wazuh-manager -m 640 "$CERT_SRC/root-ca.pem"     "$CERT_DST/root-ca.pem"
install -o wazuh-manager -g wazuh-manager -m 640 "$CERT_SRC/wazuh-1.pem"     "$CERT_DST/manager.pem"
install -o wazuh-manager -g wazuh-manager -m 640 "$CERT_SRC/wazuh-1-key.pem" "$CERT_DST/manager-key.pem"
...
# Own everything written above (config, certs, keystore) as the runtime user.
